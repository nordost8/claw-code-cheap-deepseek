...
/// DeepSeek OpenAI-compatible API (`DEEPSEEK_API_KEY`); uses `ProviderKind::OpenAi` in routing.
const DEEPSEEK_OPENAI_PROVIDER: ProviderMetadata = ProviderMetadata {
...
const MODEL_REGISTRY: &[(&str, ProviderMetadata)] = &[
    ...
    ("deepseek-chat", DEEPSEEK_OPENAI_PROVIDER),
    ("deepseek-reasoner", DEEPSEEK_OPENAI_PROVIDER),
    ("ds-chat", DEEPSEEK_OPENAI_PROVIDER),
    ("reasoner", DEEPSEEK_OPENAI_PROVIDER),
    ("r1", DEEPSEEK_OPENAI_PROVIDER),
];
...
pub fn metadata_for_model(model: &str) -> Option<ProviderMetadata> {
    ...
    if matches!(
        ...
        return Some(DEEPSEEK_OPENAI_PROVIDER);
    }
...
pub fn model_token_limit(model: &str) -> Option<ModelTokenLimit> {
    ...
    match canonical.as_str() {
        ...
        // Per DeepSeek pricing docs (128K context; chat 8K out, reasoner 64K out).
